...
int main(int argc, char const *argv[])
{
	if(argc > 1)
	{	
		//Carregando o arquivo "payout table".
		double tabela_preco[15][16];
		std::ifstream filepay("doc/payout.dat");
		if( filepay.is_open() )
		{
			std::string strImput;
			int i = 0, j = 0;
			while(filepay >> strImput)
			{
		...
		if( ifile.is_open() )
		{
			...
			KenoBet kb;
			cash_type ic, net_balance; //initial credit //saldo líquido
			int nr, i = 0; //número de rodadas
			std::string strImput;
